...
;// CONCATENATED MODULE: external "ffjavascript"
const external_ffjavascript_namespaceObject = require("ffjavascript");
;// CONCATENATED MODULE: ./src/mimcsponge.ts
/* eslint-disable camelcase */



const F = new external_ffjavascript_namespaceObject.ZqField(external_ffjavascript_namespaceObject.Scalar.fromString("21888242871839275222246405745257275088548364400416034343698204186575808495617"));
const SEED = "mimcsponge";
const NROUNDS = 220;
function mimgGetIV(seed) {
  let _seed = seed;
  if (typeof _seed === "undefined") _seed = SEED;
  const c = (0,openlogin_utils_namespaceObject.keccak256)(`${_seed}_iv`);
  const cn = external_ffjavascript_namespaceObject.Scalar.fromString(new (external_bn_js_default())(c.slice(2), 16).toString());
  const iv = cn.mod(F.p);
  return iv;
}
function mimcGetConstants(seed, nRounds) {
  ...
  let c = (0,openlogin_utils_namespaceObject.keccak256)(SEED);
  for (let i = 1; i < _nRounds; i += 1) {
    c = (0,openlogin_utils_namespaceObject.keccak256)(c);
